...
def load_aws_catalog():
    ...
    catalog_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "aws_catalog.json"
    )
    try:
        with open(catalog_path, "r") as f:
            return json.load(f)
    except FileNotFoundError:
        raise FileNotFoundError(f"AWS catalog not found at {catalog_path}")


def find_best_fit(cpu, ram):
    """Find the best AWS instance SKU that fits the requested CPU and RAM.
