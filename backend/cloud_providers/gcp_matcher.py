"""
GCP Instance Matcher
Finds the best GCP Compute Engine instance based on CPU and RAM requirements.
"""
import json
import os

class GCPInstanceMatcher:
    def __init__(self, catalog_path=None):
        if catalog_path is None:
            catalog_path = os.path.join(os.path.dirname(__file__), 'gcp_instance_catalog.json')
    ...
    def find_best_instance(self, cpu, ram):
        """
        Find the best matching GCP instance.
        ...
        if ram_to_cpu_ratio > 6:
            ...
            preferred_categories = ['memory', 'general', 'compute']
        elif ram_to_cpu_ratio < 3:
            # Compute-intensive workload
            preferred_categories = ['compute', 'general', 'memory']
        else:
            # Balanced workload
            preferred_categories = ['general', 'compute', 'memory']
    ...
    def calculate_costs(self, cpu, ram, storage, network_mbps, backup_gb):
        ...
        # Calculate storage cost (using Balanced Persistent Disk)
        storage_cost = storage * self.storage_pricing['pd_balanced_per_gb_month']
        
        # Calculate network cost
        # Estimate monthly data transfer: Mbps * 3600 * 24 * 30 / 8 / 1024
        monthly_gb = network_mbps * 3600 * 24 * 30 / 8 / 1024
        network_cost = monthly_gb * self.network_pricing['egress_per_gb']
        
        # Calculate backup cost (using Standard storage)
        backup_cost = backup_gb * self.backup_pricing['standard_per_gb_month']
